...
CREATE TABLE Bed(	
	...
	PRIMARY KEY (RoomType_code,"space"),
...
CREATE TABLE Room(	
	...
	FOREIGN KEY (Branch_code) REFERENCES Branch(code) ON DELETE SET NULL DEFERRABLE,
    	FOREIGN KEY (RoomType_code) REFERENCES RoomType(code) ON DELETE SET NULL DEFERRABLE,
	FOREIGN KEY (Branch_code,Zone_name) REFERENCES Zone(Branch_code,"name") ON DELETE SET NULL DEFERRABLE
);
...
CREATE TABLE Supply(	
	...
	FOREIGN KEY (Branch_code) REFERENCES Branch(code) ON DELETE SET NULL DEFERRABLE,
	FOREIGN KEY (SupplyType_code) REFERENCES SupplyType(code) ON DELETE SET NULL DEFERRABLE,
	FOREIGN KEY (Branch_code,Room_number) REFERENCES Room(Branch_code,"number") ON DELETE SET NULL DEFERRABLE
);
...
CREATE TABLE BillService(
	...
	FOREIGN KEY (ServiceType_name) REFERENCES ServiceType("name") ON DELETE SET NULL DEFERRABLE,
...
CREATE TABLE Service(
	...
	CONSTRAINT 	CHK_code_type CHECK ((code LIKE 'DVR%' AND "type" = 'R')
					OR (code LIKE 'DVS%' AND "type" = 'S')
					OR (code LIKE 'DVC%' AND "type"= 'C')
					OR (code LIKE 'DVM%' AND "type"= 'M')
					OR (code LIKE 'DVB%' AND "type"= 'B'))
);
CREATE TABLE Spa(
	Service_code	VARCHAR(6)
			CHECK (Service_code LIKE 'DVS%'),
	"spa"		VARCHAR(30),
	PRIMARY KEY (Service_code, "spa"),
...
IS q SYS_REFCURSOR;
BEGIN
	OPEN q FOR SELECT ST."name", ST.NoCustomer, BS.startingdate, (BS.startingdate + ST.NoDate), (SYSDATE - BS.startingdate)
	FROM ServiceType ST INNER JOIN BillService BS ON ST."name" = BS.ServiceType_name
...
DECLARE 
        priceSerivice DECIMAL (10,0);
                typeCustomer INT;
BEGIN
        SELECT price INTO priceSerivice FROM ServiceType  WHERE ServiceType."name" =  :NEW.ServiceType_name;
        SELECT "type" INTO typeCustomer FROM Customer WHERE Customer.code = :NEW.Customer_code;
        IF INSERTING THEN
        :NEW."sum" := CASE
...
DECLARE 
        ...
        ServiceType_v ServiceType."name"%type;
        Customer_v Customer.code%type;
        type_v Customer."type"%type;
BEGIN
        SELECT RoomType_code INTO roomtype_v FROM ROOM 
        WHERE :New.Branch_code = Branch_code AND :New.Room_number = "number";
        ...
        WHERE ServiceType_v = "name";
        
        SELECT Customer_code INTO Customer_v FROM Booking 
        WHERE :New.Booking_code = code;
        SELECT "type" INTO type_v FROM Customer 
...
BEGIN
        UPDATE Customer
        SET "point" = "point" + FLOOR(:NEW."sum"/1000)
...
BEFORE INSERT OR UPDATE OF "point" ON Customer
FOR EACH ROW
BEGIN
        :NEW."type" := CASE
            WHEN :NEW."point" <50 THEN 1
            WHEN :NEW."point" <100 AND :NEW."point" >=50 THEN 2
            WHEN :NEW."point" <1000 AND :NEW."point" >=100 THEN 3
            ELSE 4
        END;
END;
/
